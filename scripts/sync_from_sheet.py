...
def main() -> None:
    ...
    if not github_token:
        ...
        bulk_add_changes = [c for c in detected_changes if c["type"] == "add" and not c.get("existing_branch")]
        other_changes = [c for c in detected_changes if not (c["type"] == "add" and not c.get("existing_branch"))]

        if len(bulk_add_changes) > 1:
            branch_name = f"sync/add-bulk-{int(time.time())}"
            print(f"Processing {len(bulk_add_changes)} bulk additions on branch '{branch_name}'...")
            ...
            for change in bulk_add_changes:
                temp_yaml_events.append(change["event_data"])
            ...
            print("  Re-generating events.json for bulk additions...")
            try:
                subprocess.run(
                    [sys.executable, str(PROJECT_ROOT / "scripts" / "generate_events_json.py")],
            ...
            subprocess.run(
                ["pre-commit", "run", "--files", "data/events.yaml", "src/data/events.json"],
                cwd=str(PROJECT_ROOT), capture_output=True, check=False
            )

            run_git_cmd(["git", "add", "data/events.yaml", "src/data/events.json"])
            commit_msg = f"feat: sync {len(bulk_add_changes)} new events from Google Sheet"
            ...
            pr_title = commit_msg
            event_details = "\\n".join(
                f"- **Event ID**: `{c['id']}` | **Title**: `{c['title']}` | **Date**: `{c['date']}`"
                for c in bulk_add_changes
            )
            pr_body = (
                f"This PR was automatically raised to sync {len(bulk_add_changes)} new events "
            ...
            try:
                ...
                print(f"  Error creating pull request for bulk additions: {e}", file=sys.stderr)

            detected_changes = other_changes
